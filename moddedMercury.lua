...
function Library:create(options)
	...
	do
		...
		if S then
			...
			Event:connect(function()
				local Input
                Input = core.InputBegan:connect(function(Key)
					if (Key.UserInputType == Enum.UserInputType.MouseButton1 or Key.UserInputType == Enum.UserInputType.Touch) and UserInputService:GetFocusedTextBox() == nil then
						UserInputService.MouseBehavior = Enum.MouseBehavior.LockCurrentPosition
                        local ObjectPosition = Vector2.new(Mouse.X - core.AbsolutePosition.X, Mouse.Y - core.AbsolutePosition.Y);
                        
						while RunService.RenderStepped:wait() and touching == true do

							if Library.LockDragging and Library.Toggled then
							...
							elseif not Library.LockDragging and Library.Toggled then
								core:tween{
                ...
                local InputChanged
                InputChanged = core.InputChanged:connect(function(Key)
                    if (Key.UserInputType == Enum.UserInputType.MouseButton1 or Key.UserInputType == Enum.UserInputType.Touch)
                     ...
                     then 
                        local cha
                        cha = Key.Changed:Connect(function()
	...
	local closeButton = core:object("ImageButton", {
		...
		Image = "http://www.roblox.com/asset/?id=4991505223",
	...
	local toggleGui = self:object("ScreenGui", {
		Parent = (RunService:IsStudio() and LocalPlayer.PlayerGui) or game:GetService("CoreGui"),
		ZIndexBehavior = Enum.ZIndexBehavior.Global
	})
	
	local _toggleHolder = toggleGui:object("Frame", {
		AnchorPoint = Vector2.new(1, 0.5),
		BackgroundTransparency = 1,
		Position = UDim2.new(0.9, 0, 0.63, 0),
		Size = UDim2.new(0, 100, 0, 40)
	})

	local _toggleButton = _toggleHolder:object("TextButton", {
		Name = "Hide Script",
	...
	local _toggleButtonText = _toggleButton:object("TextLabel", {
    ...
    do
		local S, Event = pcall(function()
			return _toggleHolder.MouseEnter
		end)

		if S then
			_toggleHolder.Active = true;

			Event:connect(function()
				local Input
                Input = _toggleHolder.InputBegan:connect(function(Key)
					if (Key.UserInputType == Enum.UserInputType.MouseButton1 or Key.UserInputType == Enum.UserInputType.Touch) and UserInputService:GetFocusedTextBox() == nil then
						UserInputService.MouseBehavior = Enum.MouseBehavior.LockCurrentPosition
                        local ObjectPosition = Vector2.new(Mouse.X - _toggleHolder.AbsolutePosition.X, Mouse.Y - _toggleHolder.AbsolutePosition.Y);
                        
						while RunService.RenderStepped:wait() and touching == true do

							if Library.LockDragging then
								local FrameX, FrameY = math.clamp(Mouse.X - ObjectPosition.X, 0, gui.AbsoluteSize.X - _toggleHolder.AbsoluteSize.X), math.clamp(Mouse.Y - ObjectPosition.Y, 0, gui.AbsoluteSize.Y - _toggleHolder.AbsoluteSize.Y)
								_toggleHolder:tween{
									Position = UDim2.fromOffset(FrameX + (_toggleHolder.Size.X.Offset * _toggleHolder.AnchorPoint.X), FrameY + (_toggleHolder.Size.Y.Offset * _toggleHolder.AnchorPoint.Y)),
									Length = Library.DragSpeed
								}
							else
								_toggleHolder:tween{
									Position = UDim2.fromOffset(Mouse.X - ObjectPosition.X + (_toggleHolder.Size.X.Offset * _toggleHolder.AnchorPoint.X), Mouse.Y - ObjectPosition.Y + (_toggleHolder.Size.Y.Offset * _toggleHolder.AnchorPoint.Y)),
									Length = Library.DragSpeed	
								}
							end	
						end
                ...
                local InputChanged
                InputChanged = _toggleHolder.InputChanged:connect(function(Key)
                    if (Key.UserInputType == Enum.UserInputType.MouseButton1 or Key.UserInputType == Enum.UserInputType.Touch)
                        and UserInputService:GetFocusedTextBox() == nil
                     then 
                        local cha
				...
				local Leave
				Leave = _toggleHolder.MouseLeave:connect(function()
					Input:disconnect()
					Leave:disconnect()
				end)
			end)
	...
	local function closeUI()
		...
		core:tween({Size = UDim2.new()}, function()
			...
			toggleGui.AbsoluteObject:Destroy()
		end)

		local s,e = pcall(function() getgenv()._unloaded = true end)
		local ss,ee = pcall(function() _unloaded = true end)
	end
    ...
    creditsTab:credit{Name = "Buang #5516", Description = "AA Script Developer", Discord = "Buang#5516"}
...
function Library:dropdown(options)
	...
	if options.StartingState and table.find(options.Items, options.StartingState) then
		selectedText.Text = options.StartingState
		selectedText:tween{Size = UDim2.fromOffset(selectedText.TextBounds.X + 20, 20), Length = 0.05}
		options.Callback(options.StartingState)
	end
		...
		UserInputService.InputBegan:Connect(function(key, gameProcessed)
			if listening and not UserInputService:GetFocusedTextBox() then
				if key.UserInputType == Enum.UserInputType.Keyboard then
					...
					keybindDisplay.Text = (tostring(_key):upper()) or "?"
